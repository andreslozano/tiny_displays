...
DESCRIPTION = "class to use rgb seeedstudio lcd display https://www.seeedstudio.com/Grove-LCD-RGB-Backlight-p-1643.html "
...
class RgbLcd:
	...
	# Clear screen
	def clear(self):
		self.writeCommand(LCD_CLEAR_DISPLAY)
		
	# Return to home(top-left corner of LCD)
	def home(self):
		self.writeCommand(LCD_RETURN_HOME)
		time.sleep(self.delay * 10) # this command takes a long time!

	# set backlight to (R,G,B) (values from 0..255 for each)
	def setRGB(self, rgb=(0,0,0)):
		self.bus.write_byte_data(LCD_RGB_ADRESS,0,0)
		self.bus.write_byte_data(LCD_RGB_ADRESS,1,0)
		self.bus.write_byte_data(LCD_RGB_ADRESS,0x08,0xaa)
		self.bus.write_byte_data(LCD_RGB_ADRESS,4,rgb[0])
		self.bus.write_byte_data(LCD_RGB_ADRESS,3,rgb[1])
		self.bus.write_byte_data(LCD_RGB_ADRESS,2,rgb[2])

	# send command to display (no need for external use)    
	def writeCommand(self, cmd=None):
		self.bus.write_byte_data(LCD_TEXT_ADRESS, LCD_COMMAND_MODE, cmd)
	...
	def noDisplay(self):
		self.writeCommand(LCD_DISPLAY_CONTROL | LCD_DISPLAY_OFF)

	def display(self):
		self.writeCommand(LCD_DISPLAY_CONTROL | LCD_DISPLAY_ON)

	# Turns the underline cursor on/off
	def noCursor(self):
		self.writeCommand(LCD_DISPLAY_CONTROL | LCD_CURSOR_OFF)

	def cursor(self): 
		self.writeCommand(LCD_DISPLAY_CONTROL | LCD_CURSOR_ON)

	# Turn on and off the blinking cursor
	def noBlink(self):
		self.writeCommand(LCD_DISPLAY_CONTROL | LCD_BLINK_OFF)

	def blink(self):
		self.writeCommand(LCD_DISPLAY_CONTROL | LCD_BLINK_ON)

	# These commands scroll the display without changing the RAM
	def scrollDisplayLeft(self, times=1, speed=0.05):
		for i in xrange(times):
			self.writeCommand(LCD_CURSOR_SHIFT | LCD_DISPLAY_MOVE | LCD_MOVE_LEFT)
			lcd.pause(speed)
			

	def scrollDisplayRight(self, times=1, speed=0.05):
		for i in xrange(times):
			self.writeCommand(LCD_CURSOR_SHIFT | LCD_DISPLAY_MOVE | LCD_MOVE_RIGHT)
			lcd.pause(speed)

	# This is for text that flows Left to Right
	def leftToRight(self):
		self.writeCommand(LCD_ENTRY_MODE_SET | LCD_ENTRY_LEFT)

	# This is for text that flows Right to Left
	def rightToLeft(self):
		self.writeCommand(LCD_ENTRY_MODE_SET | LCD_ENTRY_RIGHT)

	# This will 'right justify' text from the cursor
	def autoscroll(self):
		self.writeCommand(LCD_ENTRY_MODE_SET | LCD_ENTRY_SHIFT_INCREMENT)

	# This will 'left justify' text from the cursor
	def noAutoscroll(self):
		self.writeCommand(LCD_ENTRY_MODE_SET | LCD_ENTRY_SHIFT_DECREMENT)
	...
	def showSpecialChars(self, chars=[0]):
		...
		self.bus.write_i2c_block_data(LCD_TEXT_ADRESS, LCD_TEXT_MODE, chars)
			
	# Print text/string into screen only ascii characters
	def showText(self, text="", align=0):
		if align:text = self.alignText(text, align)
		data = map(ord, text)	
		self.bus.write_i2c_block_data(LCD_TEXT_ADRESS, LCD_TEXT_MODE, data)
			
	# Typewriter style	
	def typeWriter(self, text="", delay=0.1, align=0):
		if align:text = self.alignText(text, align)
		for c in text:
			self.bus.write_byte_data(LCD_TEXT_ADRESS, LCD_TEXT_MODE, ord(c))
			time.sleep(delay)
	...
	def createChar(self, location=0, pattern=None):
		...
		self.writeCommand(LCD_TEXT_MODE | (location << 3))
		self.bus.write_i2c_block_data(LCD_TEXT_ADRESS, LCD_TEXT_MODE, pattern)
